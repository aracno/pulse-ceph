...
func TestShouldNotifyAfterCooldown(t *testing.T) {
	...
	t.Run("cooldown disabled allows first-time notification", func(t *testing.T) {
		// Cooldown=0 with no prior notification: the first-time alert
		// should still fire.
		m := newTestManager(t)
		m.mu.Lock()
	...
	// TestShouldNotifyAfterCooldown_DisabledSuppressesRenotification guards
	// against the regression in #1444: with cooldown disabled the alert
	// evaluation loop must not re-notify on every metric tick. After the
	// first-time notification, subsequent calls return false.
	...
	t.Run("negative cooldown suppresses re-notification", func(t *testing.T) {
		...
		if m.shouldNotifyAfterCooldown(alert) {
			t.Error("expected false: negative cooldown is treated the same as 0 (disabled)")
		}
	})
